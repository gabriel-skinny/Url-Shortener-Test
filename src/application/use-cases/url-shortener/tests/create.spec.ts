...
const makeSut = () => {
    ...
    const createShortUrlUseCase = new CreateShortUrlUseCase(shortUrlRepository, userExistsByIdUseCase);
...
describe("Create short url use case", () => {
    ...
    it("Should throw an error with user passed does not exists", async () => {
        const { shortUrlRepository, createShortUrlUseCase, userExistsByIdUseCase } = makeSut();
        ...
        expect(createShortUrlUseCasePromise).rejects.toStrictEqual(new NotFoundError("User not found"))
    });
