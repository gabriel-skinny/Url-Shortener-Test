...
export class LoginUseCase {
    ...
    async execute({ email, password }: ILoginUseCaseParams): Promise<ILoginUseCaseReturn> {
        ...
        if (!user.password_hash.isTheSameValue(password)) throw new WrongValueError("Password does not match");

        const { token } = await this.jwtService.generateToken({ id: user.id, name: user.name, email: user.email });
